    ...
    endpoints: (builder) => ({
        ...
        >({
            query: (requestParameters) => ({
                ...
                body: {
                    exclude_deprecated:
                        requestParameters?.exclude_deprecated || false,
                    exclude_community:
                        requestParameters?.exclude_community || false,
                },
            }),
            transformResponse: (response: Asset[]) => {
                return response.reduce(
                    (acc: { [key: string]: Asset }, asset) => {
                        return {
                            ...acc,
        ...
        getTokenPools: builder.query<Pool[], string>({
            query: (token_address) => `/pools/${token_address}`,
        }),

        getPoolsPairs: builder.query<PoolPair[], string>({
            query: (token_contract_address) =>
                `/pools-pairs/for/${token_contract_address}`,
        }),

        getAllPoolsPairs: builder.query<PoolPair[], void>({
            query: () => '/pools-pairs',
        }),
        ...
        provideLiquidity: builder.query<
            TransactionData,
            ProvideLiquidityRequest
        >({
            query: ({
...
export const {
    ...
    useGetPoolsQuery,
    useGetPoolsPairsQuery,
    useGetAllPoolsPairsQuery,
    useGetTokenPoolsQuery,
