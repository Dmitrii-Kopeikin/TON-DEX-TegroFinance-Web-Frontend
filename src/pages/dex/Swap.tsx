...
export default function SwapPage() {
  ...
  const { simulateState, showSimulateError } = useSimulateSwap({
  ...
  const { token0Balance: fromAssetBalance, token1Balance: toAssetBalance } = usePairBalances({
  ...
  useEffect(() => {
    ...
    if (swapAction === "offer") {
      setToAmount(parseFloat(Coins.fromNano(simulateState.askUnits, toAsset.decimals).toString()));
    } else {
      setFromAmount(
        parseFloat(Coins.fromNano(simulateState.offerUnits, fromAsset.decimals).toString())
      );
    }
  }, [simulateState]);
  ...
  useEffect(() => {
    if (!fromAsset || !fromAssetKey) {
      setFromAmount(0);
      return;
    }
    setFromAmount((prev) => Number(prev.toFixed(fromAsset.decimals)));
  }, [fromAssetKey]);

  useEffect(() => {
    if (!toAsset || !toAssetKey) {
      setToAmount(0);
      return;
    }
    setToAmount((prev) => Number(prev.toFixed(toAsset.decimals)));
  }, [toAssetKey]);
  ...
  const debouncedUpdateFromAmount = useDebounce(updateFromAmount, INPUT_DEBOUNCE);
  ...
  const setMaxAmount = () => {
    setFromAmount(Number(fromAssetBalance.toString()));
    setSwapAction("offer");
  };
  ...
  return (
    <Container>
      <Row className="justify-content-md-center">
        <Col lg={7} xl={5}>
          <Card className="p-2">
            ...
            >
              <Card.Title className="card-title fs-24 fw-700 me-auto">{t("swap.swap")}</Card.Title>
              <SettingsModal
                slippageTolerance={slippageTolerance}
            ...
            <Form className="p-4 pt-3">
              <Form.Group className="mb-4">
                <Form.Label className="d-flex justify-content-between mb-2 px-1">
                  ...
                  {wallet && (
                    <div
                      className="text-end small fw-500 ms-auto color-grey"
                      onClick={setMaxAmount}
                      style={{ cursor: "pointer" }}
                    >
                      {t("swap.balance")} {fromAssetBalance.toString()} {fromAsset?.symbol ?? ""}
                    </div>
                  )}
                </Form.Label>
                  ...
                  <Form.Control
                    ...
                    {...register("fromAmount", {
                      onChange: (event) => {
                        ...
                        debouncedUpdateFromAmount(parseFloat(event.target.value) || 0);
                      },
                ...
                <Form.Label className="d-flex justify-content-between mb-2 px-1">
                  ...
                  {wallet && (
                    <div className="text-end small fw-500 ms-auto color-grey">
                      {t("swap.balance")} {toAssetBalance.toString()} {toAsset?.symbol ?? ""}
                    </div>
                  )}
                </Form.Label>
                <InputGroup className="mb-3">
              ...
              <ListGroup className="list-unstyled bg-light p-3 rounded-8 mb-4">
                {fromAsset && toAsset && (
                  <ListGroup.Item className="d-flex mb-2">
                    ...
                    <span className="color-grey">
                      {`${(realPrice ?? "0").toFixed(fromAsset?.decimals || 9).slice(0, 15)} ${
                        fromAsset?.symbol
                      } ${t("swap.per")} 1 ${toAsset?.symbol}`}
                    </span>
                  </ListGroup.Item>
                )}
                <ListGroup.Item className="d-flex mb-2">
                  <span className="me-auto fw-500">{t("swap.slippageTolerance")}</span>
                  <span className="color-grey">{`${slippageTolerance}%`}</span>
                </ListGroup.Item>
                {toAsset && (
                  <ListGroup.Item className="d-flex mb-2">
                    <span className="me-auto fw-500">{t("swap.minimumReceived")}</span>
                    <span className="color-grey">
                ...
                <ListGroup.Item className="d-flex mb-2">
                  <span className="me-auto fw-500">{t("swap.priceImpact")}</span>
                  <span className="color-grey">
                    <PriceImpact priceImpact={simulateState.priceImpact} />
                  </span>
                </ListGroup.Item>
                <ListGroup.Item className="d-flex mb-2">
                  <span className="me-auto fw-500">{t("swap.blockchainFee")}</span>
                  <span className="text-muted">0.08-0.3 TON</span>
                </ListGroup.Item>
                <ListGroup.Item className="d-flex mb-2">
                  <span className="me-auto fw-500">{t("swap.dexFee")}</span>
                  <span className="text-muted">
                    {Coins.fromNano(simulateState.tonFeeUnits, 9).toString()} TON
                  </span>
                </ListGroup.Item>
                {fromAsset && toAsset && (
                  <ListGroup.Item className="d-flex mb-2">
              ...
              <>
                {wallet ? (
                  ...
                  ) : !!fromAsset && !!toAsset && fromAmount > 0 && toAmount > 0 ? (
      ...
      <SelectAssetModal
        setCurrentAssetKey={setFromAssetKey}
        otherCurrentAssetKey={toAssetKey}
        setOtherAssetKey={setToAssetKey}
        isFromModal={true}
        modalId={fromAssetModalId}
      />
      <SelectAssetModal
        setCurrentAssetKey={setToAssetKey}
